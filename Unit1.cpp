...
if ((NazwaPlikuSave.SubString(Dlugo-2,3)).UpperCase()=="GFT")
   {
    ...
    plik << XMax-1 << ',' << YMax-1 << endl; // zapis rozmiarów
        ...
        for (unsigned __int32 X = 0; X < XMax; X++, Application->ProcessMessages())
            {
             ...
             if (Kolor <= RGB(255,255,0) && Kolor > RGB(128,128,0))      Pixel = "E"; // ¿ó³ty
             if (Kolor <= RGB(0,0,128) && Kolor > RGB(255,255,0))        Pixel = "1"; // niebieski
             if (Kolor <= RGB(0,0,255) && Kolor > RGB(0,0,128))          Pixel = "9"; // niebieski jasny
             if (Kolor <= RGB(0,255,255) && Kolor > RGB(0,128,128))      Pixel = "B"; // b³êkit
             */// to na górze nie dzia³a dobrze bo nie ma prawa wiêc poprawia to to na dole
...
if ((Bevel1->Width-2) > Image1->Width && (unsigned __int32)Bevel1->Height>=Bevel1->Width*YMax/XMax) // powiêkszanie
   {
    Image1->Height = (Bevel1->Width-2)*YMax/XMax;
    Image1->Width  = Bevel1->Width-2;
    Image1->Top    = ((Bevel1->Height-2)-Image1->Height)/2;
    return;
   }
